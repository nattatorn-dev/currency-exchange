...
app.use(
  config.apiBank.proxy,
  proxy( config.apiBank.endpoint, {
...
app.use( ( req, res ) => {
  ...
  match(
    { routes: allRoutes, location: req.url },
    ( error, redirectLocation, renderProps ) => {
      if ( redirectLocation ) {
        ...
        store
          .runSaga( waitAll( preloaders ) )
          .done.then( () => {
            global.navigator = { userAgent: req.headers[ 'user-agent' ] }

            const htmlComponent = (
              <Html assets={assets} component={rootComponent} store={store} />
            )
            const renderedDomString = ReactDOMServer.renderToString(
              htmlComponent,
            )
