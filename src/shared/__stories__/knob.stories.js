...
stories.add( 'with all knobs', () => {
  ...
  const comfortTemp = number( 'Comfort Temp', 72, {
    range: true,
    min: 60,
    max: 90,
    step: 1,
  ...
  return (
    <div style={style}>
      ...
      <p>
        My most comfortable room temperature is
        {' '}
        {comfortTemp}
        {' '}
        degrees Fahrenheit.
...
stories.add( 'dates Knob', () => {
  ...
  return (
    <ul
      style={{ listStyleType: 'none', listStyle: 'none', paddingLeft: '15px' }}
    >
      <li>
...
stories.add( 'dynamic knobs', () => {
  ...
  return (
    <div>
      ...
      {showOptional === 'yes'
        ? <div>{text( 'optional', 'I can disapear' )}</div>
        : null}
    </div>
  )
} )
